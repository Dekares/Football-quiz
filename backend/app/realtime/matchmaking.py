...
def random_partner(conn: sqlite3.Connection, club_id: int) -> dict[str, Any] | None:
    """Verilen kulüple ortak oyuncusu olan rastgele bir kulüp döndür (düello auto-pick).

    `club_pair_stats` ön-hesaplanmış pair tablosunu kullanır; bu kulüp tabloda yoksa None.
    """
    c = conn.cursor()
    c.execute(
        """
        SELECT other FROM (
            SELECT club_b_id AS other FROM club_pair_stats WHERE club_a_id = ?
            UNION
            SELECT club_a_id AS other FROM club_pair_stats WHERE club_b_id = ?
        )
    ...
    if not row:
        return None
    c.execute("SELECT club_id, name, logo_url FROM clubs WHERE club_id = ?", (row[0],))
    r = c.fetchone()
    return {"club_id": r[0], "name": r[1], "logo_url": r[2]} if r else None
