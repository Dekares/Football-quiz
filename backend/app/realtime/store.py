...
Stack asyncio (socket.io AsyncServer, async_mode="asgi"); eventlet yok. Kayıt
defteri yalnızca event-loop thread'inden değiştirilir — DB sorguları ayrı thread
havuzunda koşar ama bu yapıya dokunmaz. RLock, kod üretim döngüsü gibi kısımlar
için savunma amaçlıdır (tek worker varsayımı).
"""
...
class LobbyRegistry:
    ...
    def bind_sid(self, sid: str, code: str, player_id: str) -> None:
        with self._lock:
            ...
            lob = self._lobbies.get(code.upper())
            if lob:
                lob.last_activity = time.time()
