...
def pair_is_valid(conn: sqlite3.Connection, club_a: int, club_b: int) -> bool:
    """İki kulüpte de oynamış en az bir oyuncu var mı? (seçim doğrulaması)"""
    if club_a == club_b:
        return False
    c = conn.cursor()
    c.execute(
...
def pick_reveal_player(
    conn: sqlite3.Connection, club_a: int, club_b: int
) -> dict[str, Any] | None:
    """Tur sonunda gösterilecek kayda değer bir ortak oyuncu (efsane/yüksek değer öncelikli)."""
...
def verify_free_answer(
    ...
    # Sonra: her kelime search_name'de geçiyorsa (DB'deki normalize ile birebir).
    # Kelime sayısı sınırlı → tek istekle yüzlerce LIKE üreten sorgu engellenir.
    norm_row = c.execute("SELECT normalize(?)", (text,)).fetchone()
    norm = norm_row[0] if norm_row and norm_row[0] else ""
    words = norm.split()[:MAX_ANSWER_WORDS]
    if not words:
        return None
